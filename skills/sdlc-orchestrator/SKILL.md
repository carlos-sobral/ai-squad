...
[TEAM: idea-researcher + software-architect] (discovery, optional)
  ...
  → Tech Lead approves → merge
       ↓ if environments.staging.provider != none:
       ↓   deploy to staging → [STAGING GATE: qa e2e + promotion smoke run against staging URL] → promote to prod
       ↓ if staging.provider == none:
       ↓   deploy straight to prod
  → post-deploy health check (against production)
...
**Design system sync (per module that changes the component library):** When a UI module adds, removes, or visually changes components, `product-designer` runs in **Design System Sync Mode** after the merge, republishing the project's real component library to its Claude Design design-system project (`docs/design/ds-project/` → the project id registered in `CLAUDE.md ## Tooling > design_system_project_id`). This is what lets the *next* module's artboards be assembled from components that actually exist rather than authored fresh — fidelity becomes structural instead of reproduced downstream. Two constraints to enforce in the dispatch: the target is **this repo's own project** (a name resembling the product is not ownership — an account holds starter kits and other people's systems, and each project creates its own), and the published stylesheet is **derived from the code**, never transcribed from `docs/design-system.md`. Skip for modules that ship no component change. Dormant for projects with no UI.
...
Several gates in this flow can be overridden by the Tech Lead accepting the risk: the staging gate, a `MANUAL_PENDING` smoke on a mandatory invariant, a **High** security finding waived for merge, or a missing security control flagged by `security-engineer`. "In writing with a follow-up issue" is the floor, not the bar — left there, the quality of the record depends on how zealous the agent happens to be that day. The acceptance is only valid when recorded as a **Security Exception** carrying ALL of these fields:

- **scope** — the gate / rule / finding being excepted, plus system and environment
- **owner** — the accountable person (not "the team")
- **rationale** — why the control cannot be satisfied now
- **compensating control** — what reduces the risk in the meantime, or the explicit words "none — accepted as-is"
- **approval authority** — who approved. Tech Lead for High / standard gates; escalate to a higher authority for anything touching a Critical surface
- **expiry** — a date, OR a concrete review/reassessment condition. **An exception with no expiry is invalid.** If a calendar date cannot be set, a review date plus a reassessment trigger MUST be — open-ended risk acceptance is forbidden
- **follow-up** — the action required before expiry (issue link, remediation task)

Record it at `docs/security-exceptions/<date>-<slug>.md` (or, if the project has no such dir, in the PR body) and link it from the PR. **An exception past its expiry is invalid** — the gate it covered silently re-blocks until the exception is renewed or the underlying control is satisfied; the orchestrator treats an expired exception as no exception. **Critical-severity findings are not exceptable by this path** — account takeover, data breach, privilege escalation, or a failing mandatory-invariant test (PII isolation, multi-tenant scope, data-loss) block unconditionally and must be fixed, not accepted. When the expiry is a calendar date, offer the Tech Lead a `/schedule` of the follow-up review on that date.

## External policy sources (optional, project-declared)

A project may declare external policy sources its agents must honor — an org's security rules, an architecture-standards repo, a compliance baseline. This keeps org-specific policy **out** of the universal agent definitions while still letting agents enforce it: the framework ships agnostic, the policy is the project's to supply. Declared in the project's `CLAUDE.md ## Tooling` as:

```yaml
policy_sources:
  - name: <label>
    scope: security | architecture | infra | quality | docs
    location: <path, repo, or URL the agent can read>
    mandatory: true | false
```

When present, the review-team agent whose domain matches a source's `scope` (`security-engineer` for `security`, `cloud-architect` for `infra`, `software-architect` for `architecture`, `quality-architect` for `quality`, `tech-writer` for `docs`) loads that source at the start of its review and treats its mandatory rules as **additional gates**, on top of its built-in checklist. Rules of conduct:

- **Stricter wins.** Where an external rule and a built-in default disagree, apply the more restrictive one; never let an external source *loosen* a built-in guard.
...
| `docs/design/ds-project/` | product-designer (Design System Sync Mode) | após módulo que muda a component library | projeto tem component library |
...
The `deliberate-confrontation` skill runs a **structured attack/rebuttal exchange** between the author of an artifact and a challenger with a genuinely different role perspective, so the Tech Lead decides with both sides argued — not a single self-reviewed angle. It is the *interactive* complement to the `review-team` fan-out: the fan-out gives breadth (N angles in parallel, worst-verdict-wins); confrontation gives depth (attack + rebuttal) on the few decisions that warrant it.

**You own the triggers.** The skill runs the confrontation; you + the Tech Lead own the verdict. It is **triggered, never routine** — do not add it to every module. Run it when ANY of these fires:

| Trigger | Where it fires | Challenger (attacks the author) |
|---|---|---|
| **T3 module** (multi-subsystem, public contract, regulated) | Creation phase | Per the skill's angle table |
| **Real ambiguity declared** — PRD or tech spec flags open questions that materially change the shape | Creation phase | Per the skill's angle table |
| **High cost of error** — Risk Surface `critical`/`full`, external audience, regulated domain, payments/PII | Creation + Review | Per the skill's angle table |
| **Recurring blocker** — the same failure class appeared in 3+ modules (from retros) | Retrospective → next module | Per the skill's angle table |
| **Tech Lead requests it** | Any point | Per the skill's angle table |

**Boundary (non-negotiable):** the confrontation **informs — it never decides.** It produces a recommendation; the verdict stays with you + the Tech Lead. Never wrap an objective verification gate (CI, perf, tests, a `MANUAL_PENDING` invariant) in a confrontation — those have a fact, not a debate. And the angles must be genuinely different roles (product-vs-technical-vs-marketing), never N copies of the same agent — copies share the same bias and only reduce variance, not systematic error (same rule as the judge-panel convention below).

**Where it plugs into the flow:** in the **creation phase**, run it after the author's self-review (idea brief Step 5b, PRD, tech spec) and before the artifact locks in — the author defends, the challenger attacks the premises, the author rebuts, you synthesize, the Tech Lead decides. In the **review phase**, it complements the `review-team` fan-out on the high-cost-of-error subset. In the **retrospective gate**, a recurring blocker (3+ modules) that triggered a confrontation should produce a diff to the owning agent's definition. Save the record to `docs/agents/deliberate-confrontation/YYYY-MM-DD-{slug}.md` per the skill's output format.
...
When the Clarify gate closes, the spec is complete enough for downstream agents to run without per-stage Tech Lead approval. **Default to autonomous when the project has a vision doc** (`docs/vision.md` or `docs/vision-*.md` — the residual-stop list can be derived). State the autonomous path as the recommended default:
...
If autonomous is chosen, generate the `/goal` prompt per the anatomy and operational limits in `~/.claude/CLAUDE.md` (Goal-driven autonomy section). Required sections in order: canonical docs, current state, goal-specific particularities the orchestrator can't infer, autonomy policy (loops with retry caps), residual-stop list, definition of done. The residual-stop list MUST be derived from the project's vision doc principles (`docs/vision.md` or `docs/vision-*.md`) — autonomy stops at vision-principle tensions, manual human-only actions (third-party signups, OAuth consent, payment authorization), and unconverged retry loops.
...
Before closing the retro, ask whether this module changed the component library — a component added, removed, or visually altered. If it did and `CLAUDE.md ## Tooling > design_system_project_id` is registered, `product-designer` (Design System Sync Mode) runs before the next module's UX spec; otherwise the next module's artboards get drawn against a library snapshot that no longer matches the code, which is the drift this chain exists to prevent. If the id is **not** registered and the project has a component library worth prototyping from, propose the first sync — which creates the project — as a retro action item rather than skipping it in silence.
