...
- **Never expose internal error details in API responses:** In `catch` blocks of API route handlers, never include `String(err)`, `err.message`, or stack traces in the response body — these expose ORM query details, schema information, and internal paths. Return a generic user-facing message and log the full error server-side.
- **Extract date components from strings directly, not via Date parsing:** Parsing a `YYYY-MM-DD` string with `new Date(str)` produces a UTC midnight Date. Calling `.getMonth()` or `.getFullYear()` on that Date in a server running in a timezone behind UTC returns the wrong value. Extract components directly from the string: `str.slice(0, 7)` gives `"YYYY-MM"` without any timezone conversion.
