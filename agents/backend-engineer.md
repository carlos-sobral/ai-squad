...
- **Decimal fields serialize as strings — type the frontend accordingly:** ORM Decimal fields (Prisma `Decimal`, etc.) must be serialized as strings (`.toFixed(2)`) in API responses to avoid float precision loss. Frontend types for these fields must be `string`, not `number`. Parse with `parseFloat()` at consumption sites. Never declare a monetary field as `number` in a frontend type that receives its value from a JSON API response.
...
- **When validating a cross-entity relationship, check properties on BOTH entities — not just one:** When a mutation involves a link between two entities with different types/states (e.g., cross-cycle parentId), always validate the relevant property on each entity independently. Checking only the target entity ("parent cycle must be ANNUAL") while omitting the source entity check ("child cycle must be QUARTERLY") silently allows invalid combinations that satisfy only half the contract. Example: `if (child.cycleType !== 'QUARTERLY') return 422` must come before `if (parent.cycleType !== 'ANNUAL') return 422`.
