...
- **For features that introduce entity scoping (avatar_id, tenant_id, user_id), the Observability contract MUST declare a mismatch-detection alert.** Specify: (1) every event carrying the entity_id is stamped at the canonical source of truth, not inferred by a callback that may go stale; (2) an alert that fires when the entity_id stamped on emitted events diverges from the entity_id currently active (read fresh from the source of truth) for longer than N seconds. This alert is the canary for stale-state bugs — whenever two sources of truth diverge, the mismatch lights up. Calibrate N to discriminate legitimate switch transitions (short) from persistent stale state (sustained divergence). Without this alert, FR-003-class isolation bugs only surface in user-reported incidents, after data has already mixed.
- **When a diff introduces a new external input channel (chat bot, voice STT, webhook, future Slack/Meet/email), code review MUST include an explicit assertion that an integration test covers cross-entity routing (msg arriving for entity B while entity A is the "active" one).** Do not trust that PII/scope isolation built into the legacy single-active-entity path will carry over — every new input channel doubles the surface where the routing-to-entity decision is made. Pattern to require: setup with 2+ entities configured, simulate inbound from channel for non-active entity, assert the work was attributed to the correct entity (DB row stamped with target entity_id, not active entity_id) AND that the active entity's history is untouched. Without this assertion, FR-003-class regressions slip through review — observed twice consecutively (Goal #1 multi-avatar Mac path hotfix, Goal #2 Telegram external channel loop #1).
- **Every IPC event emitted by a privileged runtime (Rust daemon, native sidecar, service worker) MUST have a corresponding listener documented in the JS-side spec.** The spec template lists, for each `runtime.emit("ns://event")` in the privileged side, the JS listener that consumes it AND the bus event it re-emits (if any) AND the EventLogger allowlist entry (if persisted). An emit without a listener is observability dead code — telemetry the privileged side promised never reaches the persistence layer. Discovered via post-merge audit: `telegram://message-sent` emitted by Rust but no JS listener → `latency_ms_send` (Rust-authoritative) never persisted; `telegram://auth-check-passed` emitted but no listener → kill-switch audit trail broken. Pattern: spec table with columns `[privileged emit path | JS listener file:line | bus event re-emitted | persisted via allowlist?]`. Code review greps for each privileged-side `emit("ns://*")` and verifies the matching JS-side `listen("ns://*")` exists.
- **Specs that touch secrets-bearing external boundaries require a cross-process persistence test, not just an in-process roundtrip.** When the design involves credentials (OAuth tokens, API keys, refresh tokens) stored in a platform vault (macOS Keychain, Windows Credential Manager, Linux Secret Service, HashiCorp Vault), the spec's Test Plan MUST require a test that proves the credential survives process boundary, not merely write-then-read-from-the-same-Entry-instance. The same Entry instance often caches the value in memory and returns it from cache, masking a broken storage layer. Two equivalent test patterns: (a) write in Entry-A, drop Entry-A, construct fresh Entry-B with same service+account, read — must succeed; (b) write in child process, exit, read in second child process — must succeed. The CI-skipped variant ("skip in CI because codesign required") does not count — flag in spec that pre-merge manual run in local dev is gating. Caught via post-merge smoke: `keyring v3.6.3` on macOS silently passed all roundtrip tests (same Entry) but failed across processes — daemon never spawned workers in production.
- **Features that add or modify IPC commands (Tauri invoke, Electron preload, Pyodide bridge, Web Worker postMessage) require a real-runtime smoke test before merge — JS mocks of the IPC layer mask entire classes of bugs.** Integration tests that mock `invoke()` accept any args case, never exercise the runtime's pool/connection management, and never simulate the platform-specific serialization quirks (Tauri camelCase conversion, Electron contextBridge cloning, etc). Concrete failure modes observed: (a) connection-pool ORM that splits a transaction across N invokes — fine in vitest (single connection), fails in real runtime (BEGIN in conn A, COMMIT in conn B); (b) IPC arg case mismatch (passing `{ avatar_id: x }` when runtime auto-converts to `{ avatarId: x }`) — mocks accept both, real runtime rejects with "missing required key"; (c) WebView CSS quirks that don't surface in jsdom/Chromium tests. Pattern to require in tech-spec Test Plan for any module that touches IPC: "smoke real no [runtime] obrigatório pre-merge — Tech Lead executa após FE entregar, validar [list of acceptance criteria visualmente]". Without this, the module can pass every automated gate (typecheck, unit, integration, lint, build) and ship completely broken in production. Caught Goal #2 Telegram retroactively: `telegram_register_bot` UI never worked in production for entire goal lifecycle until next module's smoke discovered it.
- **Tech-spec F-* flags (scope-restriction caveats) MUST be paired with an explicit "Fora-de-scope" subsection listing concrete file:line locations or behaviors that the flagged fix does NOT cover.** Pattern observed Goal #2.5: tech-spec FR-002 root cause analysis flagged "F-1: B-03 fix tem escopo restrito a `model_id` + `appearance_change_count` em App.tsx. Voice/cost campos ficam follow-up". FE interpreted flag as informational; reviewers interpreted as accepted scope. Smoke revealed AvatarSwitcher (consumer of same legacy state) also broken — fix had broader scope than flag implied but narrower than user expectation. Cost: 1 remediation loop entirely to extend the scope after the bug was discovered downstream. Required spec format: F-* flag in §X must have a sub-table "Behaviors NOT covered by this fix: [file:line | symptom | follow-up owner]". Reviewers grep for `Fora-de-scope:` and verify any consumer downstream of the flagged legacy state is either covered or explicitly listed.
- **Per-entity services constructed at module boot MUST resolve their entity_id dynamically — never capture an entity_id reference at construction time.** Anti-pattern: a long-lived singleton (session lifecycle, per-tenant cache, per-user audit logger) that takes `entityId` as a constructor arg and stores it as a field; the field freezes the value at the moment of construction, so subsequent switches in the active entity leak old-entity context into new-entity operations. Concrete failure mode observed twice: (a) Goal #1 multi-avatar hotfix `128be89` — sessions accidentally shared across Avatars because `SessionLifecycle` was constructed with `primaryAvatar.id`; switching to another Avatar kept reading the primary's session. (b) Goal #2.6 regression — same bug returned via `createSessionLifecycle(repos, ..., avatarRow?.avatar_id ?? null)` in `useBackend.ts`; the captured ID locked the lifecycle to Samantha (boot primary), causing Duca's messages to leak Samantha's context via shared `session_id`. Required pattern in tech-spec for any per-entity service: either (a) construct with `entityId=null` and resolve dynamically via `repo.getActiveEntity()` on each call, OR (b) re-construct the service on every entity switch with the new ID. Reviewers grep code-review diff for `new EntityService(..., entityId)` and verify the entity_id source is fresh per-call, not captured at boot. PII isolation gate.
- **`prop ?? fallback` where `prop` may be `boolean` (not nullish) silently ignores the fallback when prop=false.** Anti-pattern: external/internal state merge written as `const merged = externalProp ?? internalState` when the caller passes a `boolean` (not `boolean | undefined`). Nullish coalescing only falls through on `null`/`undefined`; `false` is preserved. Symptom: external boolean state appears "stuck" at `false`, internal state changes are masked. Concrete failure: Goal #2.6 `App.tsx:217` passed `externalConfigOpen={configOpenFromSwitcher}` (always boolean), `MainLayout.tsx:166` did `externalConfigOpen ?? showConfigInternal` — Settings button toggled `showConfigInternal` but merged `showConfig` stayed `false`. Reviewers must flag whenever a `??` operator merges a boolean-typed prop with internal state and require either (a) typing the prop as `boolean | undefined` and conditionally spreading it (`{...(condition ? { prop: true } : {})}`), OR (b) using explicit override semantics (`externalProp === undefined ? internalState : externalProp` with prop type `boolean | undefined`). Anti-pattern signature for code review grep: ` \?\? .*State` near JSX prop merge.
...
### Spec completeness checklist — additions (from Goal #2.7 broadcast blockers)
...
### Risk Surface taxonomy — additions (from Goal #2.7 broadcast blockers)
...
### Spec completeness checklist — additions (from Avatar Sprint 3.x persist fix blockers)
...
### Spec completeness checklist — additions (from Goal #1 Module #1 refactor Tauri-only blockers)
...
### Spec completeness checklist — additions (from Goal #2.8a spike blockers)
