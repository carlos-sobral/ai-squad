...
**Software supply-chain controls.** On every PR that adds, bumps, or newly consumes a dependency: (a) a lockfile is present and committed — no floating ranges resolved at build time; (b) a dependency vulnerability scan runs against the lockfile (`pnpm audit --prod`, `trivy fs`, or `osv-scanner`) and fails on HIGH/CRITICAL with no triaged exception; (c) for projects that ship container images or published artifacts, generate an SBOM (`syft`, `cdxgen`) and scan the built image before promotion (`trivy image`), failing promotion on Critical with no approved exception; (d) for that same class of projects, emit **signed build provenance** next to the SBOM. Provenance is an attestation that binds the artifact to the commit, the builder, and the build parameters, so a consumer can prove the artifact came from the pipeline they think it did. An SBOM answers *what is inside* the artifact; provenance answers *where the artifact came from* — a tampered build passes an SBOM scan cleanly, which is exactly why the two are not interchangeable. On GitHub Actions use the [`actions/attest-build-provenance`](https://docs.github.com/en/actions/security-for-github-actions/using-artifact-attestations/using-artifact-attestations-to-establish-provenance-for-builds) step and verify with `gh attestation verify PATH/TO/ARTIFACT -R <org>/<repo>` (`oci://<image>` for container images). Because the provenance is generated and signed by the hosted platform rather than by the build script, this is the shape [SLSA Build L2](https://slsa.dev/spec/v1.0/levels) describes — record the level the project claims in the CI/CD ADR rather than asserting a level you have not verified. Pin third-party CI actions to a commit SHA, not a moving tag. Record the chosen tools and the exception path in the CI/CD ADR.
...
### 11b. Release strategy — deploying is not releasing

Topology (section 11) answers *where* the build lands. It does not answer *who sees it*. Those are separate decisions, and collapsing them is what makes a bad deploy an incident instead of a rollback: with a single all-at-once release, the blast radius of a regression is 100% of users, and the only recovery is a revert-and-redeploy race. **Declare the release strategy in Módulo 0** in `CLAUDE.md ## Tooling > environments > release`, next to the topology.

**Three shapes, in increasing cost:**

1. **`strategy: all-at-once`** — the deploy *is* the release. Correct for local-first / single-user products, internal tools, and anything where the topology is already `staging.provider: none`. Do not impose exposure control on a project whose blast radius is one person. This is the default; it must still be a written choice, not an omission.
2. **`strategy: flagged`** — code deploys continuously to production; exposure is gated by a feature flag, so merge-to-main and user-visible release are decoupled. This is the shape that makes trunk-based development safe, and the one to reach for first when a module is user-facing and reversible ([Flagsmith on progressive delivery](https://www.flagsmith.com/blog/progressive-delivery)). Record where flags live and **who removes them** — a flag with no removal owner becomes permanent dead branching.
3. **`strategy: progressive`** — exposure expands in steps (canary → percentage ramp → 100%) while **guardrail metrics** are watched, and contracts automatically when they degrade ([canary vs. progressive delivery](https://www.getunleash.io/blog/canary-release-vs-progressive-delivery)). Reserve for surfaces where a bad release is expensive and traffic is large enough for a canary slice to carry signal. A percentage ramp with no guardrail metric and no automatic contraction is not progressive delivery — it is a slower outage.

**What you must wire when the strategy is `flagged` or `progressive`:**

- **Named guardrail metrics with a numeric abort threshold** — error rate, p95 latency, and at least one business signal, each with the value at which the rollout halts. Reuse the SLIs already declared for the `performance-engineer` gate rather than inventing a second set.
- **An automatic rollback or contraction path**, wired to those thresholds. A rollback that requires a human to notice a dashboard is not a rollback path; name the mechanism (traffic weight revert, flag kill-switch) and prove it works before the first real rollout.
- **A defined bake time per step** — how long a stage must hold clean before it expands. Without it, "ramped to 100%" only means the pipeline ran fast.
- **The DORA counting rule**, so the metrics stay honest: count deployment frequency at the **first production deploy** (even at 0% exposure), and lead time at the point the change reaches its rollout threshold. Counting both at 100% exposure silently penalises exactly the practice that reduces risk ([feature flags and DORA metrics](https://www.minware.com/blog/feature-flags-dora-metrics)).

**Deliverable:** populate `CLAUDE.md ## Tooling > environments > release` (strategy, guardrail metrics with thresholds, rollback mechanism, flag-removal owner), and record the choice and its rationale in the CI/CD ADR. In review mode, a PR that introduces a user-facing surface under a `flagged`/`progressive` strategy without a declared guardrail threshold or rollback path is a **Blocker** — the strategy is nominal until the abort condition exists.
...
- **Release strategy must be declared alongside the topology** (`CLAUDE.md ## Tooling > environments > release`). Deploying and releasing are separate decisions; `all-at-once` is a legitimate choice but must be a written one. When the strategy is `flagged` or `progressive`, the guardrail metrics, their abort thresholds, and the rollback mechanism ship with it — a ramp without an abort condition is not progressive delivery. See section 11b.
...
**Path:** `.claude/team-events/{scope}/events.jsonl`, relative to the project root. `{scope}` comes in your dispatch prompt (e.g. `review-team-g19`); if it is missing, use `solo-cloud-architect-{YYYY-MM-DD}`. Create the directory if needed. Never delete, truncate, or rewrite the file — append only. (The directory name is historical: it holds solo runs too.)

**Two lines are mandatory** — one when you start, one when you finish:

```bash
mkdir -p .claude/team-events/{scope}
printf '%s\n' '{"ts":"2026-08-22T14:32:00Z","agent":"cloud-architect","event":"started","payload":{"scope":"PR #482"}}' >> .claude/team-events/{scope}/events.jsonl
# ... your work ...
printf '%s\n' '{"ts":"2026-08-22T15:20:03Z","agent":"cloud-architect","event":"completed","payload":{"verdict":"approved-with-conditions","blockers":0,"warnings":2}}' >> .claude/team-events/{scope}/events.jsonl
```
