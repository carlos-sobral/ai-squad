...
**Path:** `.claude/team-events/{scope}/events.jsonl`, relative to the project root. `{scope}` comes in your dispatch prompt (e.g. `review-team-g19`); if it is missing, use `solo-product-manager-{YYYY-MM-DD}`. Create the directory if needed. Never delete, truncate, or rewrite the file — append only. (The directory name is historical: it holds solo runs too.)

**Two lines are mandatory** — one when you start, one when you finish:

```bash
mkdir -p .claude/team-events/{scope}
printf '%s\n' '{"ts":"2026-08-22T14:32:00Z","agent":"product-manager","event":"started","payload":{"scope":"PR #482"}}' >> .claude/team-events/{scope}/events.jsonl
# ... your work ...
printf '%s\n' '{"ts":"2026-08-22T15:20:03Z","agent":"product-manager","event":"completed","payload":{"verdict":"approved-with-conditions","blockers":0,"warnings":2}}' >> .claude/team-events/{scope}/events.jsonl
```
