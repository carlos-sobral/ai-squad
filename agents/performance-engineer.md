...
### Profiling — locating the cost, not just measuring it

Every metric above tells you **how much**. None of them tells you **where the time or memory went**, and a gate that reports "p95 regressed to 480ms" without a location hands the engineer a search problem instead of a defect. Profiling is the missing layer: metrics answer *what*, traces answer *where in the system*, profiles answer *which code* ([Grafana, continuous profiling in production](https://grafana.com/blog/continuous-profiling-in-production-a-real-world-example-to-measure-benefits-and-costs/)).

**On any regression you flag as Critical or Warning, attach a location or say explicitly that you could not obtain one.** A regression reported without a location is an observation, not a finding — the engineer who receives it will start by reproducing the measurement you already made.

- **Backend CPU / allocation:** capture a profile under the load that reproduces the regression, not at idle. Language-native samplers are enough for a gate — `pprof` (Go), `py-spy` (Python), `--cpu-prof` / `--heap-prof` (Node), `async-profiler` (JVM). Read it as a [flame graph](https://en.wikipedia.org/wiki/Flame_graph): width is total time in a call path, not call depth — the widest frame is the cost, not the deepest one.
- **Differential reading, not absolute.** For a *regression*, the useful artifact is the diff between the baseline profile and the current one; a single flame graph shows where the time goes, a differential flame graph shows what **changed**. Capture the baseline at the pre-change revision — and confirm the revision (`git log --oneline -1`), because a baseline profile taken on the wrong commit looks entirely valid.
- **Frontend:** the analogue is the Chrome DevTools Performance trace and the Long Animation Frames API — for an INP or TBT regression, name the long task and its attributed script, not just the millisecond count.
- **Continuous profiling (recommend, do not require).** When the project already runs an observability stack, always-on production profiling (Pyroscope, Elastic Universal Profiling, Datadog/GCP profilers) costs on the order of ~1% CPU and removes the "we cannot reproduce it in staging" dead end. Recommend it in the tech spec for backends with real production traffic; it is **not** a gate prerequisite, and its absence is an observation, never a FAIL.
- **Never report a profile as a measurement.** A profile explains a number; it does not replace it. The PASS/FAIL verdict stays anchored to the declared thresholds and the field data.
